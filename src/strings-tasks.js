...
function formatTime(minutes, seconds) {
  const minFormat = String(minutes).padStart(2, '0');
  const secFormat = String(seconds).padStart(2, '0');
...
function isPalindrome(str) {
  const getStr = str.toLowerCase().replace(/[\W_]/g, '');
  return getStr === getStr.split('').reverse().join('');
}
...
function findLongestWord(sentence) {
  ...
  let longest = '';
  words.forEach(function compareLength(word) {
...
function reverseWords(str) {
  ...
  words.forEach(function revWords(word) {
...
function extractNameFromTemplate(value) {
  const startInd = value.indexOf('Hello, ') + 'Hello, '.length;
  const endInd = value.indexOf('!');

  if (startInd === -1 || endInd === -1) {
    return null;
  }
  return value.slice(startInd, endInd);
}
